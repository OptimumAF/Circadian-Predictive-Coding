...
def run_circadian_sweep(
    ...
    candidates: list[dict[str, Any]] = []
    candidate_params = [
        (256, 0.01, 12, 0.12, 2, 0.80, 0.65, 2, 2, 96, 768),
        (256, 0.02, 12, 0.12, 2, 0.80, 0.65, 2, 2, 96, 768),
        (256, 0.03, 12, 0.15, 2, 0.80, 0.68, 2, 2, 96, 768),
        (384, 0.01, 12, 0.12, 2, 0.82, 0.68, 2, 2, 128, 1024),
        (384, 0.02, 12, 0.12, 2, 0.82, 0.68, 2, 2, 128, 1024),
        (384, 0.03, 12, 0.15, 2, 0.80, 0.68, 2, 2, 128, 1024),
        (384, 0.03, 12, 0.15, 1, 0.80, 0.70, 2, 2, 128, 1024),
        (512, 0.01, 12, 0.12, 2, 0.82, 0.68, 2, 2, 128, 1024),
        (512, 0.02, 12, 0.15, 2, 0.80, 0.70, 2, 2, 128, 1024),
        (512, 0.03, 12, 0.18, 1, 0.80, 0.70, 2, 2, 128, 1024),
        (512, 0.03, 14, 0.18, 1, 0.78, 0.70, 2, 2, 128, 1024),
        (512, 0.03, 14, 0.18, 1, 0.78, 0.72, 2, 3, 128, 1024),
    ]
    for index, (
        hidden,
        lr,
        ...
        min_hidden,
        max_hidden,
    ) in enumerate(candidate_params, start=1):
        override: dict[str, Any] = {
            "circadian_head_hidden_dim": hidden,
            "circadian_learning_rate": lr,
            "circadian_inference_steps": steps,
            "circadian_inference_learning_rate": inf_lr,
            "circadian_sleep_interval": sleep_interval,
            "circadian_split_threshold": split_t,
            "circadian_prune_threshold": prune_t,
            "circadian_max_split_per_sleep": max_split,
            "circadian_max_prune_per_sleep": max_prune,
            "circadian_min_hidden_dim": min_hidden,
            "circadian_max_hidden_dim": max_hidden,
        }
        config = replace(base, **override)
